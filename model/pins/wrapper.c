...
void GBsetStateLabelLong(void *model, void (*fn)()){
  // Set the state label evaluation function for long vectors to [fn].
  (*_pyCallback)("setStateLabelFn", 1, (void *)&fn);
}

void GBsetStateLabelsAll(void *model, void (*fn)()){
  // Set the state label evaluation function to [fn].
  (*_pyCallback)("setStateLabelAllFn", 1, (void *)&fn);
}
